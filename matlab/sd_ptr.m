classdef sd_ptr < handle
    ...
    methods (Access = protected)
        function obj = sd_ptr(ptr, owned, type)
            if nargin < 3
                ds = dbstack;
            ...
            if nargin < 1 || isempty(ptr)
                obj.ptr = sd_mex_gateway(obj.type, 'alloc');
                obj.owned = true;
            else
                obj.ptr = ptr;
                if nargin < 2
        ...
        function delete(obj)
            if obj.ptr ~= 0 && obj.owned
                sd_mex_gateway(obj.type, 'free', obj.ptr);
                obj.ptr = 0;
            end
        end
    end
